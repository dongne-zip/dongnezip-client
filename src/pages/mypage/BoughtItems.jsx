...
export default function BoughtItems() {
  ...
  // Fetch bought items from the server
  const fetchBoughtItems = async (page) => {
    try {
      setLoading(true);
      const response = await axios.get(`${API}/user/boughtItems?page=${page}`);
  ...
  return (
    <Container>
      ...
      {items.length > 0 ? (
        ...
        <NoItemsText>구매한 물품이 없습니다.</NoItemsText>
      )}
